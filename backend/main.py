...
},
{
  ...
  "assetAdministrationShells": [
    ...
    # to string
    json_str = json.dumps(dpp_data)

    conn = get_conn()
    cur = conn.cursor()

    # check if dff id=1 exists
    cur.execute("SELECT id FROM product_passes WHERE id = 1;")
    existing = cur.fetchone()

    if existing:
        print("Produktpass mit id=1 existiert bereits, kein Upload notwendig.")
    else:
        cur.execute("INSERT INTO product_passes (id, json_data) VALUES (1, %s);", (json_str,))
        conn.commit()
        print("Produktpass String erfolgreich hochgeladen (id=1).")

    cur.close()
    conn.close()

# Upload a product passport
@app.post("/product_pass")
async def upload_product_pass(json_data: str = Form(...)):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO product_passes (json_data) VALUES (%s) RETURNING id;",
        (json_data,)
    )
    pid = cur.fetchone()[0]
    conn.commit()
...
# Upload a document to a product passport
@app.post("/documents")
...
# Show/display a document (stream the file to the browser)
...
# file uploads to the /files directory
@app.post("/uploadfile/")
