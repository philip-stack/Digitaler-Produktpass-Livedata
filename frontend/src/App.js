...
const phases = [
  ...
  { key: "B1", label: v => `B1 - usage phase: ${v !== undefined ? v.toFixed(3) + " kg" : "n/a"}` }
];
...
function StackedCO2Bar({ values }) {
  const data = [{
    ...
    A2: values.A2,
    A1: values.A1,
    ...(values.A4 > 0 ? { A4: values.A4 } : {}),  // A4 nur bei Wert > 0
    B1: values.B1,
  }];
      ...
      {submodel.submodelElements.map((group, idx) => {
        ...
        return (
          <Box key={group.idShort || idx} sx={{ mb: 4 }}>
            <Typography
              variant="h5"
              sx={{ fontWeight: 600, mt: 2, mb: 1, fontSize: '1.13rem', color: "#111" }}
            >
              {group.idShort.replace(/_/g, " ")}
            </Typography>

            <table style={{ background: '#fcfdff', marginBottom: 12, borderCollapse: 'collapse', width: '60%', minWidth: 400 }}>
              <thead style={{ background: "#f3f7fa" }}>
                <tr>
                  <th style={{
                    textAlign: 'left',
                      ...
                      <div style={{ fontWeight: 400, color: "#444" }}>
                        <span
                          dangerouslySetInnerHTML={{
                            __html: (p.value || "").replace(
                              /WEEE/g,
                              `<a href="https://eur-lex.europa.eu/eli/dir/2012/19/oj" target="_blank" rel="noopener noreferrer" style="color:inherit; font-weight:inherit; text-decoration:underline;">WEEE</a>`
                            )
                          }}
                        />
                      </div>
...
if (submodel.idShort === 'Nameplate') {
  const nameplateExcludeIds = ["Marking_CE", "Marking_CRUUS", "Marking_RCM"];
...
export default function App() {
  ...
  const latestB1 = useLatestUsageCO2();      // ⇽ Hook
